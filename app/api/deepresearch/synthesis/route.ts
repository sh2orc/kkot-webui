...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query, modelId, analysisSteps = [] } = await request.json();

    if (!query?.trim() || !modelId || !Array.isArray(analysisSteps)) {
      return NextResponse.json({ 
        error: 'Query, modelId, and analysisSteps are required' 
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      success: true,
      synthesis: result,
      stepTitle: 'Synthesis Analysis'
    });

  } catch (error) {
    console.error('Synthesis error:', error);
    return NextResponse.json({ 
      error: 'Failed to synthesize analysis',
